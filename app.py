...
1. 🎯 [오늘 당장 방문할 Top 2 킬러 타겟 매장 지정]
   - 수집된 실제 카카오 지도 매장 리스트 중에서, 오늘 3일 무상 체험 설치 성공률이 가장 높은 **우선순위 1순위 매장과 2순위 매장(실제 상호명 2곳)**을 콕 집어 지정해 주세요.
   - 포괄적인 나열은 피하고, 왜 하필 이 2개 매장이 최고의 타겟인지(예: 약품/아세톤 냄새로 환기가 시급한 뷰티숍, 네이버 악성 리뷰 방지가 급선무인 카페 등 업종별 찌르는 Pain Point)를 다정하게 짚어주세요.

2. 🎁 [매장별 들고 갈 전용 장비 1개 & 추천 설치 위치]
   - 매장당 8대 제품 중 손에 들고 들어갈 **단 1개 핵심 장비**만 찍어주세요. (예: "1순위 매장에는 에어제닉 B타입 한 대만 들고 들어가세요")
   - 매장 내부 어디에 설치하도록 유도해야 하는지(카운터 옆, 대기실 소파 콘센트 옆 등)를 일러주세요.

3. ⏰ [점주 상주 골든타임 & 경계를 허무는 10초 킬러 오프닝 대본]
   - 사장님을 직접 만나서 대화할 확률이 가장 높은 시간대(뷰티 10~11시반, 외식 14시반~16시반, 학원 13~15시 등)를 일러주세요.
   - 사장님의 경계심을 허물고 3일 무료 체험 설치를 자연스럽게 받아내게 만드는 15년 차 베테랑만의 친근하면서도 확실한 오프닝 대본(Script)을 따옴표(" ")로 작성해 주세요.

4. 📋 [Top 2 타겟 매장 요약표 & 원클릭 길찾기 링크]
   - 1순위, 2순위 매장에 대한 통합 요약표 [우선순위 | 실제 상호명 | 추천 장비 | 점주 골든타임 | 네이버 지도 길찾기]를 작성해 주세요.
   - 형식: [📍 네이버 지도로 길찾기](https://map.naver.com/v5/search/실제상호명)
"""
...
with st.sidebar:
    ...
    st.caption("건물/주소를 입력하시면 오늘 바로 방문할 Top 2 킬러 타겟 매장과 베테랑 오프닝 대본을 챙겨드립니다.")
        ...
        if quick_rejection_prompt:
            ...
            if real_stores_data and len(real_stores_data) > 0:
                ...
                final_system_instruction += (
                    ...
                    "아래 카카오 지도 실시간 매장 리스트 중에서 **오늘 3일 무상 체험 설치 성공 확률이 가장 높은 우선순위 1순위 매장과 2순위 매장(실제 상호명 2곳)**을 콕 집어서 지정해 주세요.\n"
                    "절대로 백화점식 나열이나 포괄적 설명을 하지 마시고, 후배 플래너가 당장 문 열고 들어갈 Top 2 매장을 지정하고 친근한 대본을 일러주세요:\n"
                    f"{stores_text_list}"
                )
